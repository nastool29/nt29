...
class Config(object):
    ...
    def init_config(self):
        try:
            if not self._config_path:
                print(f"【Config】NASTOOL_CONFIG 路径：{self._config_path} 环境变量未设置，程序无法工作，正在退出...")
                quit()
            if not os.path.exists(self._config_path):
                print(f"【Config】NASTOOL_CONFIG 路径：({self._config_path}) 不存在 ...")
                ...
                print(f'【Config】config.yaml 路径：{cfg_tp_path} 配置文件不存在，已将配置文件模板复制到配置目录...')

                shutil.copytree(self.get_inner_sites_path(), self.get_sites_path())
                print(f"【Config】folder: sites/ 路径：{self.get_inner_sites_path()} 文件夹不存在，已将文件夹复制到{self.get_sites_path()}目录...")
    ...
    def get_sites_path(self):
        return os.path.join(self.get_config_path(), "sites")
    ...
    def get_inner_sites_path(self):
        return os.path.join(self.get_root_path(), "config/sites")
