...
class Media:
    ...
    def __search_tmdb(self, file_media_name,
        ...
        if search_type == MediaType.MOVIE:
            ...
            for year in year_range:
                ...
                if info:
                    ...
                    log.Logger().info("【Meta】%s 识别到 电影：TMDBID=%s, 名称=%s, 上映日期=%s" % (
                        file_media_name,
                        info.get('id'),
                        info.get('title'),
            ...
            if info:
                ...
                log.Logger().info("【Meta】%s 识别到 电视剧：TMDBID=%s, 名称=%s, 首播日期=%s" % (
                    file_media_name,
                    info.get('id'),
                    info.get('name'),
        ...
        if info:
            ...
            log.Logger().info("【Meta】%s 以年份 %s 在TMDB中未找到%s信息!" % (
    ...
    def __search_movie_by_name(self, file_media_name, first_media_year):
        ...
        try:
            ...
            log.Logger().error(f"【Meta】连接TMDB出错：{str(err)}")
            return None
        except Exception as e:
            log.Logger().error(f"【Meta】连接TMDB出错：{str(e)}")
            return None
        log.Logger().debug(f"【Meta】API返回：{str(self.search.total_results)}")
        if len(movies) == 0:
            log.Logger().debug(f"【Meta】{file_media_name} 未找到相关电影信息!")
            return {}
        else:
            info = {}
            if first_media_year:
    ...
    def __search_tv_by_name(self, file_media_name, first_media_year):
        ...
        log.Logger().debug(f"【Meta】API返回：{str(self.search.total_results)}")
        if len(tvs) == 0:
            log.Logger().debug(f"【Meta】{file_media_name} 未找到相关剧集信息!")
            return {}
        else:
            info = {}
            if first_media_year:
    ...
    def __search_tv_by_season(self, file_media_name, media_year, season_number):
        ...
        def __season_match(tv_info, season_year):
            ...
            try:
                ...
                log.Logger().error(f"【Meta】连接TMDB出错：{e1}")
                return False
            return False
        ...
        try:
            ...
            log.Logger().error(f"【Meta】连接TMDB出错：{e}")
            return None

        if len(tvs) == 0:
            log.Logger().debug("【Meta】%s 未找到季%s相关信息!" % (file_media_name, season_number))
            return {}
        else:
            for tv in tvs:
                if (self.__compare_tmdb_names(file_media_name, tv.get('name'))
    ...
    def __search_multi_tmdb(self, file_media_name):
        ...
        log.Logger().debug(f"【Meta】API返回：{str(self.search.total_results)}")
        if len(multis) == 0:
            log.Logger().debug(f"【Meta】{file_media_name} 未找到相关媒体息!")
            return {}
        else:
            info = {}
            for multi in multis:
        ...
        if info:
            ...
            log.Logger().info("【Meta】%s 在TMDB中未找到媒体信息!" % file_media_name)
            return info
    ...
    def __search_tmdb_web(self, file_media_name, mtype: MediaType):
        ...
        if StringUtils.is_chinese(file_media_name):
            return {}
        log.Logger().info("【Meta】正在从TheDbMovie网站查询：%s ..." % file_media_name)
                ...
                if len(tmdb_links) == 1:
                    ...
                    if tmdbinfo:
                        ...
                        if tmdbinfo.get('media_type') == MediaType.MOVIE:
                            log.Logger().info("【Meta】%s 从WEB识别到 电影：TMDBID=%s, 名称=%s, 上映日期=%s" % (
                                file_media_name,
                                tmdbinfo.get('id'),
                                tmdbinfo.get('title'),
                                tmdbinfo.get('release_date')))
                        else:
                            log.Logger().info("【Meta】%s 从WEB识别到 电视剧：TMDBID=%s, 名称=%s, 首播日期=%s" % (
                                file_media_name,
                                tmdbinfo.get('id'),
                                tmdbinfo.get('name'),
                    ...
                    log.Logger().info("【Meta】%s TMDB网站返回数据过多：%s" % (file_media_name, len(tmdb_links)))
                else:
                    log.Logger().info("【Meta】%s TMDB网站未查询到媒体信息！" % file_media_name)
    ...
    def get_tmdb_info(self, mtype: MediaType,
        ...
        """
        if not self.tmdb:
            log.Logger().error("【Meta】TMDB API Key 未设置！")
            return None
        if language:
            self.tmdb.language = language
        else:
    ...
    def get_media_info(self, title,
        ...
        if not meta_info.get_name() or not meta_info.type:
            log.Logger().warn("【Rmt】%s 未识别出有效信息！" % meta_info.org_string)
            return None
        if mtype:
        ...
        if not cache or not self.meta.get_meta_data_by_key(media_key):
            ...
            if not file_media_info and self._search_keyword:
                ...
                if cache_name:
                    log.Logger().info("【Meta】开始辅助查询：%s ..." % cache_name)
                    if is_movie:
        ...
        for file_path in file_list:
            try:
                if not os.path.exists(file_path):
                    log.Logger().warn("【Meta】%s 不存在" % file_path)
                    continue
                # 解析媒体名称
                # 先用自己的名称
                ...
                if not os.path.isdir(file_path) \
                    ...
                    log.Logger().info("【Meta】%s 跳过蓝光原盘文件：" % file_path)
                    continue
                # 没有自带TMDB信息
                if not tmdb_info:
                    # 识别名称
                ...
                print(str(err))
                log.Logger().error("【Rmt】发生错误：%s - %s" % (str(err), traceback.format_exc()))
        # 循环结束
    ...
    def __get_tmdb_movie_detail(self, tmdbid, append_to_response=None):
        ...
        if not self.movie:
            return {}
        try:
            log.Logger().info("【Meta】正在查询TMDB电影：%s ..." % tmdbid)
    ...
    def __get_tmdb_tv_detail(self, tmdbid, append_to_response=None):
        ...
        if not self.tv:
            return {}
        try:
            log.Logger().info("【Meta】正在查询TMDB电视剧：%s ..." % tmdbid)
    ...
    def get_tmdb_tv_season_detail(self, tmdbid, season: int):
        ...
        if not self.tv:
            return {}
        try:
            log.Logger().info("【Meta】正在查询TMDB电视剧：%s，季：%s ..." % (tmdbid, season))
    ...
    def __search_engine(feature_name):
        ...
        log.Logger().info("【Meta】推断关键字为：%s ..." % ([k[0] for i, k in enumerate(ret) if i < 4]))
        if len(ret) == 1:
            keyword = ret[0][0]
        else:
            pre = ret[0]
            ...
            if nextw[0].find(pre[0]) > -1:
                ...
                keyword = pre[0]
        log.Logger().info("【Meta】选择关键字为：%s " % keyword)
