    ...
    def filter_search_results(self, result_array: list,
        ...
        for item in result_array:
            ...
            if filter_args.get("seeders") and not indexer.public and str(seeders) == "0":
                log.Logger().info(f"【{self.index_type}】{torrent_name} 做种数为0")
                index_rule_fail += 1
                continue
            # 识别种子名称
            meta_info = MetaInfo(title=torrent_name, subtitle=description)
            if not meta_info.get_name():
                log.Logger().info(f"【{self.index_type}】{torrent_name} 无法识别到名称")
                index_match_fail += 1
                continue
            # 大小及促销等
            ...
            match_flag, res_order, match_msg = self.filter.check_torrent_filter(meta_info=meta_info,
                                                                                ...
                                                                                downloadvolumefactor=downloadvolumefactor)
            if not match_flag:
                log.Logger().info(f"【{self.index_type}】{match_msg}")
                index_rule_fail += 1
                continue
            # 识别媒体信息
            if not match_media:
                ...
                if meta_info.imdb_id \
                    ...
                    if match_media \
                        ...
                        if not media_info:
                            log.Logger().warn(f"【{self.index_type}】{torrent_name} 识别媒体信息出错！")
                            index_error += 1
                            continue
                ...
                if match_media.over_edition:
                    ...
                    if media_info.type != MediaType.MOVIE \
                        ...
                        log.Logger().info(f"【{self.index_type}】{media_info.get_title_string()}{media_info.get_season_string()} "
