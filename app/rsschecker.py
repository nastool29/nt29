...
class RssChecker(object):
    ...
    def check_task_rss(self, taskid):
        ...
        if len(rss_result) == 0:
            log.Logger().warn("【RssChecker】%s 未下载到数据" % taskinfo.get("name"))
            return
        else:
            log.Logger().info("【RssChecker】%s 获取数据：%s" % (taskinfo.get("name"), len(rss_result)))
        # 处理RSS结果
        res_num = 0
        no_exists = {}
        for res in rss_result:
            try:
                ...
                if taskinfo.get("uses") == "D":
                    ...
                    if taskinfo.get("recognization") == "Y":
                        if cache_info.get("id"):
                            ...
                            media_info = self.media.get_media_info(title=meta_name,
                                                                   mtype=mediatype)
                            if not media_info:
                                log.Logger().warn("【RssChecker】%s 识别媒体信息出错！" % title)
                                continue
                            if not media_info.tmdb_info:
                                log.Logger().info("【RssChecker】%s 识别为 %s 未匹配到媒体信息" % (title, media_info.get_name()))
                                continue
                        # 检查是否已存在
                        if media_info.type == MediaType.MOVIE:
                            exist_flag, no_exists, _ = self.downloader.check_exists_medias(meta_info=media_info,
                                                                                           no_exists=no_exists)
                            if exist_flag:
                                log.Logger().info("【RssChecker】电影 %s 已存在" % media_info.get_title_string())
                                continue
                        else:
                            exist_flag, no_exists, _ = self.downloader.check_exists_medias(meta_info=media_info,
                                                                                           no_exists=no_exists)
                    ...
                    # 未匹配
                    if not match_flag:
                        log.Logger().info(f"【RssChecker】{match_msg}")
                        continue
                    else:
                        # 匹配优先级
                        media_info.set_torrent_info(res_order=res_order)
                        if taskinfo.get("recognization") == "Y":
                            log.Logger().info("【RssChecker】%s 识别为 %s %s 匹配成功" % (
                                title,
                            ...
                            log.Logger().info(f"【RssChecker】{title}  匹配成功")
                    # 添加下载列表
                    if not enclosure:
                        log.Logger().warn("【RssChecker】%s RSS报文中没有enclosure种子链接" % taskinfo.get("name"))
                        continue
                ...
                ExceptionUtils.exception_traceback(e)
                log.Logger().error("【RssChecker】处理RSS发生错误：%s - %s" % (str(e), traceback.format_exc()))
                continue
        log.Logger().info("【RssChecker】%s 处理结束，匹配到 %s 个有效资源" % (taskinfo.get("name"), res_num))
        # 添加下载
        if rss_download_torrents:
            for media in rss_download_torrents:
                ...
                if ret:
                    ...
                    log.Logger().error("【RssChecker】添加下载任务 %s 失败：%s" % (
        ...
        if rss_subscribe_torrents:
            for media in rss_subscribe_torrents:
                ...
                if rss_media and code == 0:
                    ...
                    log.Logger().warn("【RssChecker】%s 添加订阅失败：%s" % (media.get_name(), msg))
    ...
    def __parse_userrss_result(self, taskinfo):
        ...
        if not rss_parser:
            log.Logger().error("【RssChecker】任务 %s 的解析配置不存在" % taskinfo.get("name"))
            return []
        if not rss_parser.get("format"):
            log.Logger().error("【RssChecker】任务 %s 的解析配置不正确" % taskinfo.get("name"))
            return []
        try:
            rss_parser_format = json.loads(rss_parser.get("format"))
        except Exception as e:
            ExceptionUtils.exception_traceback(e)
            log.Logger().error("【RssChecker】任务 %s 的解析配置不是合法的Json格式" % taskinfo.get("name"))
            return []
        # 拼装链接
        ...
        if rss_parser.get("params"):
            ...
            try:
                ...
                ExceptionUtils.exception_traceback(e)
                log.Logger().error("【RssChecker】任务 %s 的解析配置附加参数不合法" % taskinfo.get("name"))
                return []
        ...
        if rss_parser.get("type") == "XML":
            try:
                ...
                log.Logger().error("【RssChecker】任务 %s 获取的订阅报文无法解析：%s" % (taskinfo.get("name"), str(err)))
                return []
            ...
            try:
                ...
                ExceptionUtils.exception_traceback(err)
                log.Logger().error("【RssChecker】任务 %s 获取的订阅报文不是合法的Json格式：%s" % (taskinfo.get("name"), str(err)))
                return []
            item_list = jsonpath.jsonpath(result_json, rss_parser_format.get("list"))[0]
            if not isinstance(item_list, list):
                log.Logger().error("【RssChecker】任务 %s 获取的订阅报文list后不是列表" % taskinfo.get("name"))
                return []
            for item in item_list:
                rss_item = {}
                for key, attr in rss_parser_format.get("item", {}).items():
    ...
    def get_rss_articles(self, taskid):
        ...
        for res in rss_result:
            try:
                ...
                ExceptionUtils.exception_traceback(e)
                log.Logger().error("【RssChecker】获取RSS报文发生错误：%s - %s" % (str(e), traceback.format_exc()))
        return rss_articles
    ...
    def check_rss_articles(self, flag, articles):
        ...
        try:
            ...
            ExceptionUtils.exception_traceback(e)
            log.Logger().error("【RssChecker】设置RSS报文状态时发生错误：%s - %s" % (str(e), traceback.format_exc()))
            return False
